...
class _ProfilePage_EditState extends State<ProfilePage_Edit> {
  ...
  final List <String> cursosUminho = [
   ...
   List <String> disponibilidade =[
    ...
    "Estou disponível para oportunidades de estágio",
  ...
  if (mounted) {
    setState(() {
      ...
      cota = data['cotas'] ?? false;
      _phone.text = data['phone'] == null ? "" : data['phone'].toString();
      profile_avatar.text = data['avatar'] == null ? "" : data['avatar'].toString();
      _curso.text = data['curso'] == null ? "" : data['curso'].toString();
      _disponibilidade.text = data['disponibilidade'] == null ? "" : data['disponibilidade'].toString();
  ...
  void initState() {
    ...
    stream.listen((DatabaseEvent event) {
     ...
     if (data != null && data is Map) {
        print("Dados válidos recebidos: $data");
  ...
  void launchURL(url) async {
   ...
   // final DatabaseReference ref = FirebaseDatabase.instance.ref("users/${FirebaseAuth.instance.currentUser?.uid.trim()}"); // Referência ao Realtime Database
    ...
    await ref
        .child("curso") // Caminho no Realtime Database
        .set(curso); // Atualiza o curso 
...
Future <void> UpdateDisponibilidade (String disponibilidade) async{
  await ref.child("disponibilidade").set(disponibilidade);
  
  _disponibilidade.text = disponibilidade;

  setState(() {});
}
 Color _getColorBasedOnDisponibilidade(String text) {
    if (text == disponibilidade[0]) {
      return Color.fromARGB(255, 9, 81, 9);  // Verde, por exemplo
    } else if (text == disponibilidade[1]) {
      return Color.fromARGB(255, 195, 113, 13);  // Vermelho, por exemplo
    } else if (text == disponibilidade[2]) {
      return Color.fromARGB(255, 199, 43, 43);  // Laranja, por exemplo
    } 
   else {
                  ...
                  Expanded(
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: [
                          // Lista de cursos
                          for (String disp in disponibilidade)
                            RadioListTile<String>(
                              title: Text(disp, style: TextStyle(fontSize: 14)),
                              value: disp,
                              groupValue: _DisponibilidadeEscolhida,
                              onChanged: (String? value) {
                                setState(() {
                                  _DisponibilidadeEscolhida = value;
                                });
                              },
                              activeColor: Colors.orange,
                            ),
                    ...
                    child: Row(
                      ...
                      children: [
                        TextButton(
                          onPressed: () {
                            Navigator.of(context).pop();
                            UpdateDisponibilidade(_DisponibilidadeEscolhida!); // Atualiza no Firebase
                          },
                          child: Text("Confirmar",style: TextStyle(color:Colors.orange),),
                        ),
                        TextButton(
                                      ...
                                      child: Row(
                                        children: [
                                          Container(
                                            ...
                                            decoration: BoxDecoration(                                                 
                                                    color: _getColorBasedOnDisponibilidade(_disponibilidade.text),
                                                    borderRadius: BorderRadius.all(
                                                        Radius.circular(20))),
                                            child: Center(
                                              child: Text(
