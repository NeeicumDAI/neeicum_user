...
class w_participadosPage extends StatefulWidget {
  ...
  @override
  State<w_participadosPage> createState() => _w_participadosPageState();
}

class _w_participadosPageState extends State<w_participadosPage> {
  Map datamap = {};
  Map prev_datamap = {};
  ...
  void initState() {
    ...
    dbRef = FirebaseDatabase.instance.ref().child('jee').child('second_day');
    stream = dbRef.orderByChild('date').onValue;
    stream.listen((DatabaseEvent event) {
      updateInfo(event.snapshot.children);
    });

    dbRef = FirebaseDatabase.instance.ref().child('jee').child('third_day');
    stream = dbRef.orderByChild('date').onValue;
    stream.listen((DatabaseEvent event) {
      updateInfo(event.snapshot.children);
    });

    dbRef = FirebaseDatabase.instance.ref().child('jee').child('last_day');
    stream = dbRef.orderByChild('date').onValue;
    stream.listen((DatabaseEvent event) {
      updateInfo(event.snapshot.children);
    });
  ...
  void updateInfo(data) {
    if (mounted) {
      setState(() {
        ...
        data.forEach((child) {
          if (child.value["show"]) {
            prev_datamap[child.key] = child.value;
          }
        });
      });
    }
    datamap.addAll(prev_datamap);
  }
