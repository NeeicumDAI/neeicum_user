...
class _objGeraisPageState extends State<objGeraisPage> {
  ...
  Map objetivosfixos = {
    'hasIMG': {
      'coins': 10,
      'img':
          'https://github.com/RuiRocha23/imagens/blob/main/foto.png?raw=true',
      'name': 'Foto',
    },
    'hasCV': {
      'coins': 10,
    ...
    'hasEMP': {
      'coins': 10,
      'img':
          'https://github.com/RuiRocha23/imagens/blob/main/empresas.png?raw=true',
      'name': 'Empresas',
    },
    'hasAcred': {
      'coins': 10,
      'img':
          'https://github.com/RuiRocha23/imagens/blob/main/acred.png?raw=true',
      'name': 'Acreditação',
    },
  ...
  Future<void> checkObjetivos() async {
    ...
    DatabaseReference flagCV = FirebaseDatabase.instance
        .ref()
        .child('flags')
        .child(uid.toString())
        .child('hasCV');
    ...
    DatabaseReference flagAcred = FirebaseDatabase.instance
        .ref()
        .child('flags')
        .child(uid.toString())
        .child('hasACRED');

    final IMG = await flagIMG.get();
    final CV = await flagCV.get();
    final EMP = await allEMP.get();
    final ACRED = await flagAcred.get();

    if (IMG.exists) {
      bool flag = (IMG.value) as bool;
      if (flag) {
        objetivosfixos['hasIMG']?['reg'] = {
    ...
    if (CV.exists) {
      bool flag = (CV.value) as bool;
      if (flag) {
        objetivosfixos['hasCV']?['reg'] = {
          uid.toString(): {'appear': true}
        };
      }
    }
    ...
    if (ACRED.exists) {
      bool flag = (ACRED.value) as bool;
      if (flag) {
        objetivosfixos['hasAcred']?['reg'] = {
          uid.toString(): {'appear': true}
        };
      }
    }
